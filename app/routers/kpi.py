...
def list_kpis(organization_id: int = Depends(get_current_organization_id), db: SessionLocal = Depends(get_db)):
    return db.query(KPI).filter(KPI.organization_id == organization_id).all()

@router.post("/kpi", response_model=KPIResponse, tags=["kpi"])
def create_kpi(kpi: KPICreate, organization_id: int = Depends(get_current_organization_id), db: SessionLocal = Depends(get_db)):
...
def update_kpi(kpi_id: int, kpi: KPICreate, organization_id: int = Depends(get_current_organization_id), db: SessionLocal = Depends(get_db)):
...
def delete_kpi(kpi_id: int, organization_id: int = Depends(get_current_organization_id), db: SessionLocal = Depends(get_db)):
    db_kpi = db.query(KPI).filter(KPI.id == kpi_id, KPI.organization_id == organization_id).first()
    if not db_kpi:
        raise HTTPException(status_code=404, detail="KPI not found")
    db.delete(db_kpi)
